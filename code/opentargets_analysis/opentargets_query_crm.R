...
crm_eqtls_gtex_removed <- vroom(crm_qtl_bed_loc)
bim <- vroom(bim_loc, col_names=c("CHR", "RS", "POS_CM", "POS_BP", "ALLELE_1", "ALLELE_2")) 
opentargets_translator <- filter(bim, RS %in% crm_eqtls_gtex_removed$EB_VARIANT_ID) %>%
  mutate(variant_id=paste0(str_replace(CHR, "chr", ""), "_", POS_BP, "_", ALLELE_2, "_", ALLELE_1))

crm_eqtls_opentargets_overlap <- bind_rows(map_df(opentargets_translator$variant_id, pull_associations))
crm_eqtl_gwas_variants <- opentargets_translator %>%
  filter(variant_id %in% crm_eqtls_opentargets_overlap$variant) %>%
  pull(RS)
write(crm_eqtl_gwas_variants, snplist_loc)

crm_eqtls_gtex_removed_gwas_bed <- crm_eqtls_gtex_removed %>%
  filter(EB_VARIANT_ID %in% crm_eqtl_gwas_variants) %>%
  write_tsv(crm_qtl_gwas_bed_loc)
