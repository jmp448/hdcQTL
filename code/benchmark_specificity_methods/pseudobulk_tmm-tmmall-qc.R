...
pseudobulk <- pseudobulk %>%
  ...
  rename_with(function(x){if_else(x=="gene", true=x, false=paste0("NA", x))})

pseudobulk.mat <- pseudobulk %>%
  column_to_rownames("gene") %>%
  as.matrix

# Gene filter - filter to genes with nonzero variance, at least 50 samples with at least 10 counts
gene_vars <- rowVars(pseudobulk.mat) > 0
gene_minexp <- rowSums(pseudobulk.mat >= 10) >= 50
gene_keepers <- rownames(pseudobulk.mat)[gene_vars & gene_minexp]
pseudobulk.mat <- pseudobulk.mat[gene_keepers,]

# TMM normalization to get log TMM-normalized counts
pseudobulk.mat <- DGEList(counts=pseudobulk.mat) %>% calcNormFactors(method="TMM") %>% cpm(log=T)
pseudobulk <- as_tibble(pseudobulk.mat, rownames="gene")
...
for (d in c(cell.types)) {
  ...
  # get expression within cell type
  expression <- pseudobulk %>% 
    select(c(gene, ends_with(paste0("_", d)))) %>% 
    rename_with(function(x){if_else(x=="gene", true=x, false=str_sub(x, 1, 7))})
  expression.mat <- expression %>%
    column_to_rownames("gene") %>%
    as.matrix
  
  # Within celltype gene filter - filter to genes with nonzero variance, nonzero median expression
  gene_vars <- rowVars(expression.mat) > 0
  gene_nonzeros <- rowMedians(expression.mat) > 0
  ...
  # Within celltype expression normalization
