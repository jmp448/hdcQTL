...
for (d in c(cell.types)) {
  ...
  # gene filter - nonzero variance, median expression of over 10 UMI
  ...
  # 3. quantile normalize within each individual across all genes to standard normal
  expression.mat <- apply(expression.mat, 2, invnorm_transform)
  # 4. re-insert gene names and convert to tibble
