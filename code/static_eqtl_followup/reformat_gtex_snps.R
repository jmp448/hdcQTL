...
# snps <- separate(snps, variant_id, into=c("#CHR", "END", "GTEX_REF", "GTEX_ALT", "build"), sep="_") had to remove due to segfault errors
snps <- as.data.table(snps)
snps[, c("#CHR", "END", "GTEX_REF", "GTEX_ALT", "build") := tstrsplit(variant_id, "_", fixed = TRUE)]
snps <- as_tibble(snps)
