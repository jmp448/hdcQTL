...
gs_genes_loc = snakemake.input['gs_genes']
...
# Load gene set genes and filter phenotypes
gs_genes = pd.read_csv(gs_genes_loc, names=["gene"], header=None)['gene']
def intersection(lst1, lst2):
    lst3 = [value for value in lst1 if value in lst2]
    return lst3
observed_gs_genes = intersection(gs_genes, phenotype_pos_df.index)
phenotype_df = phenotype_df.loc[observed_gs_genes]
phenotype_pos_df = phenotype_pos_df.loc[observed_gs_genes]
