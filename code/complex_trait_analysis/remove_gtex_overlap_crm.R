...
crm_qtl_bed_loc <- snakemake@input[['crm_bed']]
gtex_bed_loc <- snakemake@input[['gtex_bed']]

snplist_loc <- snakemake@output[['non_gtex_crm']]
non_gtex_crm_loc <- snakemake@output[['non_gtex_crm_bed']]

crm_qtl_bed <- vroom(crm_qtl_bed_loc)
...
non_gtex_eqtls <- anti_join(crm_qtl_bed, gtex_overlap_bed, by=c("EB_VARIANT_ID", "EB_HGNC")) %>%
  write_tsv(non_gtex_crm_loc)
