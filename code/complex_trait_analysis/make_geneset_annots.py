...
geneset_loc = snakemake.input['geneset']
...
subset = str(snakemake.wildcards['subset'])
...
gene_list = pd.read_csv(geneset_loc, sep="\t")

# Get the genes for this subset, and filter QTL lists
if subset == "full":
    gene_list_keepers = gene_list
else:
    gene_list_keepers = gene_list.loc[gene_list['cluster'] == subset]

if variant_group == "mash-signif":
    eb_eqtls = eb_eqtls.loc[[g in list(gene_list_keepers['hgnc']) for g in list(eb_eqtls['EB_HGNC'])]]
else:
    eb_eqtls = eb_eqtls.loc[[g in list(gene_list_keepers['hgnc']) for g in list(eb_eqtls['phenotype_id'])]]
gtex_eqtls = gtex_eqtls.loc[[g in list(gene_list_keepers['ensg']) for g in list(gtex_eqtls['phenotype_id'])]]
