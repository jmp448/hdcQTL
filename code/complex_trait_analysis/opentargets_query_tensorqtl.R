...
variant_translator <- filter(bim, RS %in% celltype_eqtls$EB_VARIANT_ID) %>%
  distinct() %>%
  mutate(variant_id = paste0(str_replace(CHR, "chr", ""), "_", POS, "_", REF, "_", ALT))

# Write to a list of rsIDs
gwas_hits <-  bind_rows(map_df(variant_translator$variant_id, pull_associations))
gwas_variants <- filter(variant_translator, variant_id %in% gwas_hits$variant)
gwas <- filter(celltype_eqtls, EB_VARIANT_ID %in% gwas_variants$RS)

snplist <- dplyr::select(gwas, EB_VARIANT_ID) %>%
  distinct() %>%
  write_tsv("results/cellregmap_eqtl_calling/eb_cellid/pseudobulk_tmm/basic/tensorqtl_gwas_snplist.txt", col_names=F)
