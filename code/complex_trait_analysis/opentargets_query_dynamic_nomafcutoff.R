...
cm_translator <- filter(bim, RS %in% cm_dynamic_eqtls$EB_VARIANT_ID) %>%
  distinct() %>%
  mutate(variant_id = paste0(str_replace(CHR, "chr", ""), "_", POS, "_", ALT, "_", REF))
hep_translator <- filter(bim, RS %in% hep_dynamic_eqtls$EB_VARIANT_ID) %>%
  distinct() %>%
  mutate(variant_id = paste0(str_replace(CHR, "chr", ""), "_", POS, "_", ALT, "_", REF))
neur_translator <- filter(bim, RS %in% neur_dynamic_eqtls$EB_VARIANT_ID) %>%
  distinct() %>%
  mutate(variant_id = paste0(str_replace(CHR, "chr", ""), "_", POS, "_", ALT, "_", REF))

cm_dynamic_gwas_hits <- bind_rows(map_df(cm_translator$variant_id, pull_associations))
cm_dynamic_eqtls_gwas_variants <- filter(cm_translator, variant_id %in% cm_dynamic_gwas_hits$variant)
cm_dynamic_eqtls_gwas <- filter(cm_dynamic_eqtls, EB_VARIANT_ID %in% cm_dynamic_eqtls_gwas_variants$RS)

hep_dynamic_gwas_hits <- bind_rows(map_df(hep_translator$variant_id, pull_associations))
hep_dynamic_eqtls_gwas_variants <- filter(hep_translator, variant_id %in% hep_dynamic_gwas_hits$variant)
hep_dynamic_eqtls_gwas <- filter(hep_dynamic_eqtls, EB_VARIANT_ID %in% hep_dynamic_eqtls_gwas_variants$RS)

neur_dynamic_gwas_hits <- bind_rows(map_df(neur_translator$variant_id, pull_associations))
neur_dynamic_eqtls_gwas_variants <- filter(neur_translator, variant_id %in% neur_dynamic_gwas_hits$variant)
neur_dynamic_eqtls_gwas <- filter(neur_dynamic_eqtls, EB_VARIANT_ID %in% neur_dynamic_eqtls_gwas_variants$RS)

# Write to a list of rsIDs
cm_snplist <- dplyr::select(cm_dynamic_eqtls_gwas, EB_VARIANT_ID) %>%
  distinct() %>%
  write_tsv("results/dynamic_qtl_calling/eb-cm_15binstrimmed/pseudobulk_tmm/nipals/dynamic_gwas_snplist.txt", col_names=F)

hep_snplist <- dplyr::select(hep_dynamic_eqtls_gwas, EB_VARIANT_ID) %>%
  distinct() %>%
  write_tsv("results/dynamic_qtl_calling/eb-hep_15binstrimmed/pseudobulk_tmm/nipals/dynamic_gwas_snplist.txt", col_names=F)

neur_snplist <- dplyr::select(neur_dynamic_eqtls_gwas, EB_VARIANT_ID) %>%
  distinct() %>%
  write_tsv("results/dynamic_qtl_calling/eb-neur_15binstrimmed/pseudobulk_tmm/nipals/dynamic_gwas_snplist.txt", col_names=F)
