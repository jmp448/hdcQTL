...
sample_summary <- read_tsv("/project2/gilad/jpopp/ebQTL/data/static/highpass_cellid_all/pseudobulk-scran/fastgxc_context_centered/sample_summary_manual.tsv") %>%
...
What's the median number of cells per cell type?
```{r}
sample_summary %>%
  filter(!dropped) %>%
  filter(type %in% cell.types) %>%
  group_by(type) %>%
  summarize(med=median(n_cells)) %>%
  arrange(med) %>%
  ggplot(aes(x=type, y=med)) +
  geom_bar(stat="identity") +
  geom_abline(slope=0, intercept=95, color="red", linetype="dashed") +
  xlab("Cell Type") + ylab("Median # cells") +
  coord_flip()
```
...
prefix <- "/project2/gilad/jpopp/ebQTL/results/static/highpass_cellid_all/pseudobulk-scran/fastgxc_context_centered/"
...
```{r}
m <- readRDS("/project2/gilad/jpopp/ebQTL/results/static/highpass_cellid_all/pseudobulk-scran/fastgxc_context_centered/mashr.tophits.rds")
```
...
corrplot(Vhat, is.corr=F, tl.pos='l')
```
