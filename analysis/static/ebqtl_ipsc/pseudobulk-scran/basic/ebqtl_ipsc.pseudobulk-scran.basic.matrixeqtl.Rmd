...
tophits <- read_tsv("/project2/gilad/jpopp/ebQTL/results/static/ebqtl_ipsc/pseudobulk-scran/basic/IPSC/matrixeqtl.cis_qtl_pairs.tophits.tsv")
```
...
```{r}
ebqtl_strict_all <- read_tsv("/project2/gilad/jpopp/ebQTL/results/static/ebqtl_ipscfilt/pseudobulk-scran/basic_redo/IPSC/7pcs/matrixeqtl.cis_qtl_pairs.all.tsv") %>%
  unite(gv, gene, SNP, sep="--", remove=F)

ebqtl_strict_hits <- read_tsv("/project2/gilad/jpopp/ebQTL/results/static/ebqtl_ipscfilt/pseudobulk-scran/basic_redo/IPSC/7pcs/matrixeqtl.cis_qtl_pairs.tophits.tsv")

sum(ebqtl_strict_hits$q <= 0.1)
```
Also now going to consider TMM normalization
```{r}
ebqtl_tmm_all <- read_tsv("/project2/gilad/jpopp/ebQTL/results/static/ebqtl_ipsc/pseudobulk-tmm/basic_redo/IPSC/7pcs/matrixeqtl.cis_qtl_pairs.all.tsv") %>%
  unite(gv, gene, SNP, sep="--", remove=F)
ebqtl_tmm_hits <- read_tsv("/project2/gilad/jpopp/ebQTL/results/static/ebqtl_ipsc/pseudobulk-tmm/basic_redo/IPSC/7pcs/matrixeqtl.cis_qtl_pairs.tophits.tsv")

sum(ebqtl_tmm_hits$q <= 0.1)
```
...
```{r}
overlap_tests <- intersect(ebqtl_strict_all$gv, vqtl_all$gv)
overlap_tests <- intersect(overlap_tests, ebqtl_all$gv)
overlap_tests <- intersect(overlap_tests, ebqtl_tmm_all$gv)
vqtl_overlap <- filter(vqtl_all, gv %in% overlap_tests)  # 952623 of 990328
ebqtl_overlap <- filter(ebqtl_all, gv %in% overlap_tests) # 915000 of 1965203
ebqtl_strict_overlap <- filter(ebqtl_strict_all, gv %in% overlap_tests) # 920454 of 1768879
ebqtl_tmm_overlap <- filter(ebqtl_tmm_all, gv %in% overlap_tests)
...
ebqtl_tmm_overlap <- ebqtl_tmm_overlap %>%
  group_by(gene) %>% add_count() %>%
  mutate(bonf.p=if_else(`p-value`*n>1, 1, `p-value`*n)) %>%
  select(-n)
...
ebqtl_tmm_overlap_tophits <- ebqtl_tmm_overlap %>%
  group_by(gene) %>%
  arrange(bonf.p) %>%
  filter(!duplicated(gene))
ebqtl_tmm_overlap_tophits$q <- qvalue(ebqtl_tmm_overlap_tophits$bonf.p)$qvalues
```

```{r}
tibble("approach"=factor(c("original_vQTL", "reanalysis_vQTL", "ebQTL", "ebQTL_strict", "ebQTL_tmm"),
                         levels=c("original_vQTL", "reanalysis_vQTL", "ebQTL", "ebQTL_strict", "ebQTL_tmm")),
       "n_qtl"=c(235, sum(vqtl_overlap_tophits$q <= 0.1), 
                 sum(ebqtl_overlap_tophits$q <= 0.1), sum(ebqtl_strict_overlap_tophits$q <= 0.1),
                 sum(ebqtl_tmm_overlap_tophits$q <= 0.1))) %>%
  ggplot(aes(x=approach, y=n_qtl)) +
  geom_bar(stat="identity")
```
...
combined_stats <- ebqtl_tmm_overlap %>%
  ungroup() %>%
  mutate(se=beta/`t-stat`) %>%
  select(gv, beta, se) %>%
...
ipsc_markers <- c("POU5F1", "NANOG", "GAL", "UTF1", "GDF3", "FGF4",
 "LIN28A", "TFCP2L1", "DNMT3B", "NODAL", "TDGF1", "TRF1",
 "GDF3", "DPPA3", "DPPA5")

all_exp <- all_exp %>%
  mutate(marker=gene %in% ipsc_markers) %>%
  arrange(marker)
```
```{r fig.width=10, fig.height=10}
ggplot(all_exp, aes(x=sarkar_exp, y=eb_exp, color=marker)) +
...
Yikes, is this trend more dramatic for lower ncell samples?
```{r}
...
size_ordered <- sample_summary %>%
  arrange(desc(n_cells)) %>%
  pull(individual) %>%
  as.character

all_exp <- all_exp %>%
  mutate(individual=factor(str_sub(sample, 1, 5), levels=size_ordered))
```
...
Does this hold up after quantile normalization?
```{r}
invnorm_transform <- function(x) {
  qqnorm(x, plot.it=F)$x
}

sarkar_exp_norm <- read_tsv("/project2/gilad/jpopp/ebQTL/data/single_cell_objects/vqtl_ipsc.pseudobulk-scran.tsv") %>%
  mutate(across(!gene, invnorm_transform)) %>%
  pivot_longer(!gene, names_to="sample", values_to="sarkar_exp")

eb_exp_norm <- read_tsv("/project2/gilad/jpopp/ebQTL/data/single_cell_objects/ebqtl_ipsc.pseudobulk-scran.tsv") %>%
  mutate(across(!gene, invnorm_transform)) %>%
  pivot_longer(!gene, names_to="sample", values_to="eb_exp")
...
```
```{r fig.width=10, fig.height=10}
ggplot(all_exp_norm, aes(x=sarkar_exp, y=eb_exp, color=marker)) +
  scale_color_manual(values=c("black", "red")) +
  facet_wrap(~individual) +
  geom_point(size=1) +
  geom_abline(slope=1, intercept=0, linetype="dashed", color="red")
...
## Check what happens with strict cell filtering
```{r}
sample_summary_ebqtlfilt <- read_tsv("/project2/gilad/jpopp/ebQTL/data/static/ebqtl_ipscfilt/pseudobulk-scran/sample_summary.tsv") %>%
  mutate(individual=factor(individual), type=factor(type), study="EB")
sample_summary_vqtl <- read_tsv("/project2/gilad/jpopp/ebQTL/data/static/vqtl_ipsc/pseudobulk-scran/sample_summary.tsv") %>%
  mutate(individual=factor(individual), type=factor(type), study="Sarkar")

sample_summary_all_filt <- bind_rows(sample_summary_ebqtlfilt, sample_summary_vqtl)
```
```{r}
ggplot(sample_summary_all_filt, aes(x=individual, y=n_cells, fill=study)) +
...
sarkar_exp <- read_tsv("/project2/gilad/jpopp/ebQTL/data/single_cell_objects/vqtl_ipsc.pseudobulk-scran.tsv") %>%
  pivot_longer(!gene, names_to="sample", values_to="sarkar_exp")
eb_exp_filt <- read_tsv("/project2/gilad/jpopp/ebQTL/data/single_cell_objects/ebqtl_ipscfilt.pseudobulk-scran.tsv") %>%
  pivot_longer(!gene, names_to="sample", values_to="eb_exp")

all_exp_filt <- inner_join(sarkar_exp, eb_exp_filt, by=c("gene", "sample"))

ipsc_markers <- c("POU5F1", "NANOG", "GAL", "UTF1", "GDF3", "FGF4",
 "LIN28A", "TFCP2L1", "DNMT3B", "NODAL", "TDGF1", "TRF1",
 "GDF3", "DPPA3", "DPPA5")

all_exp_filt <- all_exp_filt %>%
  mutate(marker=gene %in% ipsc_markers) %>%
  mutate(individual=str_sub(sample, 1, 5)) %>%
  arrange(marker)
```

```{r fig.width=10, fig.height=10}
ggplot(all_exp_filt, aes(x=sarkar_exp, y=eb_exp, color=marker)) +
  scale_color_manual(values=c("black", "red")) +
  facet_wrap(~individual) +
  geom_point(size=1) +
  geom_abline(slope=1, intercept=0, linetype="dashed", color="red")
...
## Comparing sample summaries between studies
How does the number of cells per sample compare between the two studies?
```{r}
sample_summary_ebqtl <- read_tsv("/project2/gilad/jpopp/ebQTL/data/static/ebqtl_ipsc/pseudobulk-scran/sample_summary.tsv") %>%
  mutate(individual=factor(individual), type=factor(type), study="EB")
sample_summary_vqtl <- read_tsv("/project2/gilad/jpopp/ebQTL/data/static/vqtl_ipsc/pseudobulk-scran/sample_summary.tsv") %>%
  mutate(individual=factor(individual), type=factor(type), study="Sarkar")

sample_summary_all <- bind_rows(sample_summary_ebqtl, sample_summary_vqtl)
```

```{r}
ggplot(sample_summary_all, aes(x=individual, y=n_cells, fill=study)) +
  geom_bar(stat="identity", position="dodge") +
  coord_flip() +
  theme_classic(base_size=20)
```
...
How does the distribution of library sizes per sample compare?
```{r}
