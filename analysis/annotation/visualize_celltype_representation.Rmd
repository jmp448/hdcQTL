...
ggplot(umap_viz, aes(x=umap1, y=umap2, color=value)) +
...
proportions_per_celltype <- donor_celltype %>%
  ...
  select(value, donor_id, proportion) %>%
  distinct %>%
  mutate(value = factor(value, levels=color_map$type_spaced)) %>%
  drop_na() # removes rare cell types

ggplot(proportions_per_celltype, aes(x=donor_id, y=proportion, fill=value)) +
...
ggplot(atleast_5cells, aes(x=all_donors, y=n_types)) +
