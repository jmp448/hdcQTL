...
eb_sighits_i2qtl_overlap_loc <- "/project2/gilad/jpopp/ebQTL/results/static_eqtl_followup/qtl_sets/tensorqtl/original/signif_variant_gene_pairs.I2QTL.overlap.bed"
...
How many of the eQTLs that we identify have previously been identified as an eQTL by GTEx?
```{r}
...
How about in IPSCs?
```{r}
i2qtl_distinct <- vroom(eb_sighits_i2qtl_overlap_loc, col_names=c("CHR", "START", "STOP", "EB_ENSG", "EB_HGNC", 
                                                          "RSID", "CELLTYPE", "CHR1", "START1", 
                                                          "STOP1", "GTEX_ENSG", "GTEX_REF", "GTEX_ALT")) %>%
  dplyr::select(EB_HGNC, RSID) %>%
  distinct()

eb_novel_noipsc <- anti_join(eb_novel, i2qtl_distinct, by=c("EB_HGNC"="EB_HGNC", "EB_VARIANT_ID"="RSID"))
nrow(eb_novel_noipsc) / nrow(eb_distinct)
```
