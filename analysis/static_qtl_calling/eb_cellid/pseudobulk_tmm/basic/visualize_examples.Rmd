

```{r}
library(tidyverse)
library(vroom)
library(snpStats)
```
## Helper functions
This function will take a snpMatrix from SnpStats and convert it to a tibble with genotype
```{r}
snpstats2geno <- function(s) {
  ac <- tibble(ac=as.numeric(s$genotypes@.Data), sample=rownames(s$genotypes@.Data))
  ac2geno <- tibble("ac"=c(1, 2, 3), 
                    "genotype"=c(paste(rep(s$map$allele.1, 2), sep="", collapse=""), 
                                                  paste(s$map$allele.1, s$map$allele.2, sep="", collapse=""), 
                                                  paste(rep(s$map$allele.2, 2), sep="", collapse="")))
  geno <- left_join(ac, ac2geno, by="ac")
  geno
}
```


## Static eQTL examples
SCZ example
```{r}
snp_rsid <- "rs2693698"
gene_hgnc <- "BCL11B"
celltype <- "PNS-glia"
```

FAM20C (cell type proportion)
```{r}
snp_rsid <- "rs28731441"
gene_hgnc <- "FAM20C"
celltype <- "PNS-glia"
```

TMEFF2
```{r}
snp_rsid <- "rs12693642"
gene_hgnc <- "TMEFF2"
celltype <- "Ciliated-epithelial-cells"
```

```{r}
bed <- paste0("/project2/gilad/jpopp/ebQTL/data/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/", celltype, "/genotypes_filtered_plink.bed")
bim <- paste0("/project2/gilad/jpopp/ebQTL/data/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/", celltype, "/genotypes_filtered_plink.bim")
fam <- paste0("/project2/gilad/jpopp/ebQTL/data/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/", celltype, "/genotypes_filtered_plink.fam")
snpstat <- read.plink(bed, bim, fam, na.strings = c("0", "-9"), sep = "." , select.subjects = NULL, select.snps = snp_rsid)

genotypes_df <- snpstats2geno(snpstat)
```

```{r}
expression_df <- vroom(paste0("/project2/gilad/jpopp/ebQTL/data/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/", celltype, "/expression.bed.gz")) %>%
  select(-c(`#chr`, start, end)) %>%
  column_to_rownames("gene") %>% t %>%
  as_tibble(rownames="sample") %>%
  select(c(sample, gene_hgnc))
```

```{r}
inner_join(genotypes_df, expression_df, by="sample") %>%
  ggplot(aes(x=genotype, y=.data[[gene_hgnc]])) +
  geom_boxplot(alpha=0.5, width=0.5) +
  geom_jitter(aes(color=genotype), position = position_jitter(seed = 1, width = 0.1), size=3.5) +
  theme_classic(base_size=20) +
  scale_fill_discrete(name=snp_rsid) +
  #xlab("Genotype") + ylab(paste0(gene_hgnc, " Expression")) +
  xlab("") + ylab("TMEFF2 Expression\n(Ciliated Epithelial Cells)") +
  scale_color_manual(values=c("#2ca02c", "#ff7f0e", "#1f77b4")) +
  #ggtitle(celltype) +
  labs(color="Genotype at rs28731441")
```

Additionally visualize the eQTL with residualized expression
```{r}
covariate_df <- vroom(paste0("data/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/", celltype, "/expression.bed.gz")) 
```

## TMEFF2
```{r}
tmeff <- vroom("temp/tmeff_ex_data.tsv") %>%
  dplyr::select(-c(`...1`)) %>%
  filter(genotype != -1)
```

```{r}
tmeff_lm <- lm('epithelial ~ . + 0', tmeff)
plot(tmeff_lm)
```

There are two pretty clear outliers here, and they are driving the trend
```{r}
tmeff_outlier_eval <- ctprop[-c(92, 113),]
tmeff_outlier_eval_lm <- lm('epithelial ~ .', tmeff_outlier_eval)
plot(tmeff_outlier_eval_lm)
summary(tmeff_outlier_eval_lm)
```

## MYSM1
```{r}
mysm <- vroom("temp/mysm_ex_data.tsv") %>%
  dplyr::select(-c(`...1`)) %>%
  filter(genotype != -1) %>%
  relocate(epithelial)
```

```{r}
mysm_lm <- lm('epithelial ~ .', mysm)
plot(mysm_lm)
```

There are two pretty clear outliers here
```{r}
mysm_outlier_eval <- ctprop[-c(190, 259),]
mysm_outlier_eval_lm <- lm('epithelial ~ .', mysm_outlier_eval)
plot(mysm_outlier_eval_lm)
```

```{r}
mysm_outlier_eval_2 <- ctprop[-c(92, 113),]
mysm_outlier_eval_2_lm <- lm('epithelial ~ .', mysm_outlier_eval_2)
plot(mysm_outlier_eval_2_lm)
```
Removing those gets rid of the effect


