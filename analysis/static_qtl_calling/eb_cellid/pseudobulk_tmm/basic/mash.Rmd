...
```{r}
eb_sighits_loc <- "/project2/gilad/jpopp/ebQTL/results/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/8pcs/mash-signif_variant_gene_pairs.tsv"
eb_sighits_overlap_loc <- "/project2/gilad/jpopp/ebQTL/results/static_eqtl_followup/eb_cellid/pseudobulk_tmm/basic/8pcs/mash-signif_variant_gene_pairs.full_gtex_overlap.bed"
...
n_contexts <- as_tibble(get_n_significant_contexts(full_m), rownames="gv")
...
```{r}
permutation_ehits <- read_tsv("/project2/gilad/jpopp/ebQTL/results/static_qtl_calling/eb_cellid/pseudobulk_tmm/basic/8pcs/signif_variant_gene_pairs.tsv")

doubly_novel_overlap_posteriors <- filter(novel_overlap_posteriors, !gene %in% permutation_ehits$phenotype_id)
```
...
mash_comp_viz <- bind_rows(mash_ests, nonmash_ests) %>%
  mutate(approach=case_match(factor(approach, levels=c("non-mash", "mash")), "non-mash" ~ "Without MASH", "mash" ~ "With MASH")) %>%
  mutate(approach=factor(approach, levels=c("Without MASH", "With MASH"))) %>%
  mutate(celltype=factor(celltype, levels=rev(type_order)))

ggplot(mash_comp_viz, aes(x=celltype, y=beta, color=celltype)) +
