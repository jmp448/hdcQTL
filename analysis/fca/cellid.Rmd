...
celltype_sampler <- as_tibble(colData(fca)) %>%
  dplyr::select(sample, Organ, celltype) %>%
...
ggplot(emb_subset, aes(x=MCA_1, y=MCA_2)) +
  ...
  geom_label(data=centroids, aes(label=celltype, color=celltype), size=8, position=position_jitter(width=1,height=1)) +
  #geom_label(data=marker_emb, aes(label=gene), size=8, position=position_jitter(width=1,height=1)) +
