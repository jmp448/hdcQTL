...
## Calling trans-eQTLs
First, we're going to isolate some variants of interest - these are the top eQTLs on 
the eGenes lacking EB-GTEx overlap, which appear in the 'Tissue Development' GO BP gene set
```{r}
...
top_tissue_dev_qtls <- filter(eb_all, phenotype_id %in% novel_egenes_tissue_dev) %>%
  arrange(pval_nominal) %>%
  group_by(phenotype_id) %>%
  slice_head(n=1)
```

Next, we define our super positive controls - the top eQTLs on the eGenes that did have
EB-GTEx overlap, which also appear in the 'Tissue Development' GO BP gene set
```{r}
gtex_egenes_tissue_dev <- intersect(tissue_dev_genes, overlap_egenes)

top_tissue_dev_qtls_gtex <- filter(eb_all, phenotype_id %in% gtex_egenes_tissue_dev) %>%
  arrange(pval_nominal) %>%
  group_by(phenotype_id) %>%
  slice_head(n=1) %>%
  ungroup() %>%
  sample_n(nrow(top_tissue_dev_qtls))
```

Now, for a weaker positive control - a random set of EB top cis-eQTLs, matched by MAF and TSS distance
```{r}
