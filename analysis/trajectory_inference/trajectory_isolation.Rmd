...
pseudotime_plot <- ggplot(pseudotime_df, aes(x=UMAP_1, y=UMAP_2, color=pseudotime_color)) +
  ...
  geom_point(data=filter(pseudotime_df, trajectory != "none"), size=1e-7, alpha=0.5) * (blend("lighten") + blend("multiply", alpha = 0.5)) +
...
pseudo_hep_plot<- ggplot(hep_pcs, aes(x=PC1, y=PC2, color=PC1)) +
  ...
  theme_classic(base_size=15)  +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
  xlab("Pseudotime (PC1)") + ggtitle("Hepatoblast Trajectory")
```
