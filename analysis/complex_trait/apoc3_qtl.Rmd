...
betas <- vroom("results/cellregmap_eqtl_calling/eb_cellid/pseudobulk_tmm/basic/mash-signif.fasttopics_10topics.APOC3.cellregmap.betas.tsv", col_select=c("PSEUDOCELL", "BETA_GXC", "BETA_G", "EB_HGNC", "EB_VARIANT_ID"))
...
pseudocell_df <- betas_loadings %>%
  select(all_of(c("PSEUDOCELL", "APOC3_rs11604424"))) %>% 
  rename(beta_gxc = APOC3_rs11604424) %>%
  left_join(select(pseudocell_exp, c(`...1`, APOC3)), by=c("PSEUDOCELL"="...1")) %>%
  mutate(donor=str_extract(PSEUDOCELL, "[^_]+")) %>%
  left_join(crm_genotypes_df, by=c("donor")) %>%
  mutate(genotype=factor(genotype))

crm_qtl_plot <- ggplot(pseudocell_df, aes(x=beta_gxc, y=APOC3, color=genotype)) +
  ...
  ylab("APOC3 Expression") +
  labs(color=v)
crm_qtl_plot
```

```{r}
crm_qtl_plot_loess <- ggplot(pseudocell_df, aes(x=beta_gxc, y=APOC3, color=genotype)) +
  geom_point(size=0.25) * (blend("lighten") + blend("multiply", alpha = 0.5)) +
