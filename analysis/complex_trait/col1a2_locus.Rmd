...
## Dissecting early and late effects across the locus
Here, we are looking at the significance of the interaction term - this is not the same 
as the significance of the total genetic effect, which varies across contexts but also 
incorporates the main effect

To see if this is worth looking into further, look at just the effect estimates at early and
late pseudotime (these will be noisy). First, load the effect estimates
```{r}
col1a2_effects_loc <- "results/dynamic_qtl_calling/eb-cm_15binstrimmed/pseudobulk_tmm/nipals/10clpcs/tensorqtl_interactions.all.tsv"
col1a2_effects <- fread(col1a2_effects_loc)[(variant_id %in% col1a2_qtl_ld$RS) & (phenotype_id == "COL1A2"), ]
```

I also need to know the distribution of pseudotime values
```{r}
cm_pseudotimes <- vroom("data/dynamic_qtl_calling/eb-cm_15binstrimmed/pseudobulk_tmm/nipals/pseudotime.tsv")
hist(cm_pseudotimes$pseudotime)
range(cm_pseudotimes$pseudotime)
```

```{r fig.height=4, fig.width=4}
col1a2_alleffects <- col1a2_qtl_ld %>%
  left_join(dplyr::select(col1a2_effects, c(variant_id, b_g, b_gi)), by=c("RS"="variant_id")) %>%
  mutate(early_pseudotime=-7) %>%
  mutate(late_pseudotime=26)  %>%
  mutate(early_effect = b_g + map2_dbl(b_gi, early_pseudotime, `*`)) %>%
  mutate(late_effect = b_g + map2_dbl(b_gi, late_pseudotime, `*`)) %>%
  mutate(diff_effect = late_effect - early_effect) %>%
  mutate(credible_set = POS %in% gwas_strongest) %>% 
  dplyr::select(c(POS, credible_set, early_effect, late_effect, diff_effect, b_g)) %>%
  pivot_longer(cols=c(early_effect, late_effect, diff_effect, b_g),
               names_to="effect", values_to="effect estimate") %>%
  mutate(effect=factor(effect, levels=c("b_g", "diff_effect", "early_effect", "late_effect")))


ggplot(col1a2_alleffects, aes(x=POS, y=`effect estimate`, color=credible_set)) +
  geom_point() + 
