...
# save to a TSV
write_tsv(pseudobulk_scran, "/project2/gilad/jpopp/ebQTL/data/single_cell_objects/vqtl_ipsc.pseudobulk_scran.tsv")
write_tsv(pseudobulk_tmm, "/project2/gilad/jpopp/ebQTL/data/single_cell_objects/vqtl_ipsc.pseudobulk_tmm.tsv")
...
sample_summary <- as.data.frame(sce@colData@listData) %>%
  ...
  count(ind_type, name="n_cells_filtered") %>%
  ...
  write_tsv("data/static/vqtl_ipsc/pseudobulk_scran/sample_summary.tsv")
...
sample_summary <- as.data.frame(sce@colData@listData) %>%
  ...
  mutate(n_umi=NA) %>%
  relocate(ind_type, n_umi, individual, type, n_cells_filtered, dropped) %>%
  write_tsv("data/static/vqtl_ipsc/pseudobulk_tmm/sample_summary.tsv")
